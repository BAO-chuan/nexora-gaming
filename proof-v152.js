/* Nexora Gaming v1.5.2 — URL Proof Guide
   Proof-only patch. Does not modify Daily System or Achievements. */
(()=>{
  const q=id=>document.getElementById(id);
  const safe=s=>typeof esc==='function'?esc(s):String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const date=s=>typeof fmtDate==='function'?fmtDate(s):new Date(s).toLocaleString('vi-VN');
  const status=s=>typeof proofStatus==='function'?proofStatus(s):({pending:'⏳ Chờ duyệt',approved:'✓ Đã duyệt',rejected:'✕ Từ chối'}[s]||s);
  const validUrl=v=>{try{const u=new URL(v);return u.protocol==='http:'||u.protocol==='https:'}catch{return false}};
  const copy=async text=>{try{await navigator.clipboard.writeText(text);return true}catch{const t=document.createElement('textarea');t.value=text;document.body.appendChild(t);t.select();const ok=document.execCommand('copy');t.remove();return ok}};
  const guide=()=>`<button type="button" id="proofGuideToggle" class="proof-guide-btn">❓ Hướng dẫn lấy URL ảnh/video</button><div id="proofGuide" class="proof-guide hidden"><h3>📎 Cách tạo URL bằng chứng</h3><ol><li>Tải ảnh hoặc video của bạn lên một dịch vụ lưu trữ/chia sẻ mà bạn sử dụng.</li><li>Bật quyền chia sẻ để <b>người có liên kết có thể xem</b>.</li><li>Chọn <b>Sao chép liên kết / Copy link</b>.</li><li>Quay lại Nexora, dán liên kết vào ô <b>URL bằng chứng</b> rồi gửi Admin duyệt.</li></ol><div class="proof-guide-warning">⚠️ Trước khi gửi, hãy thử mở URL ở tab ẩn danh. Nếu xem được mà không cần đăng nhập thì Admin cũng có thể kiểm tra. Không gửi mật khẩu, OTP, mã khôi phục hoặc thông tin đăng nhập game.</div></div>`;
  const wireGuide=()=>{const b=q('proofGuideToggle'),g=q('proofGuide');if(b&&g)b.onclick=()=>{g.classList.toggle('hidden');b.textContent=g.classList.contains('hidden')?'❓ Hướng dẫn lấy URL ảnh/video':'✕ Đóng hướng dẫn'}};
