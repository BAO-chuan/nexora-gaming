...
function authPage(){if(!$('loginForm'))return;
 ...
 registerForm.onsubmit=async e=>{e.preventDefault();msg('authMsg','Đang tạo tài khoản...');const meta={display_name:$('registerName').value.trim()};if(refCode)meta.referral_code=refCode;const {data,error}=await db.auth.signUp({email:$('registerEmail').value.trim(),password:$('registerPassword').value,options:{data:meta}});if(error)return msg('authMsg',error.message,true);if(data?.session){msg('authMsg','✓ Đăng ký thành công. Đang vào Nexora...');setTimeout(()=>location.href='dashboard.html',500)}else{msg('authMsg',refCode?'Đăng ký thành công. Mã giới thiệu đã được ghi nhận.':'Đăng ký thành công. Bạn có thể đăng nhập ngay.')}};}
