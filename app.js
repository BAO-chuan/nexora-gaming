...
async function v24Dashboard(){if(!$('communityMissionList'))return;await Promise.all([loadNotifications(),loadCommunityMissions(),loadClanLeaderboard()]);$('refreshCommunityMissions').onclick=loadCommunityMissions;$('refreshClanLeaderboard').onclick=loadClanLeaderboard;$('markNotificationsRead').onclick=async()=>{await db.rpc('nexora_mark_notifications_read');loadNotifications()}}
